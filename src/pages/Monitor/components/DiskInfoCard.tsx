...
const PARTITION_INFO_TABLE_CLOUMNS: ColumnsType = [
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const tempDiskPercentStat: ((prevState: never[]) => never[]) | { time: string; name: string; value: number; }[] = [];
          // change data format, get value and time
          res.forEach((element) => {
  ...
  const DiskTimeStatCard: React.FC = () => {
    ...
    const [metricList, setMetricList] = useState([
      { key: 'disk_stat.partition_with_usage_stats', value: 'disk_stat.partition_with_usage_stats' },
    ]);
    ...
    return (
      <>
        ...
        {connTableData.length !== 0 && (
          <Table columns={DISK_TABLE_CLOUMNS} scroll={{ x: 1500 }} dataSource={connTableData} />
        )}
      </>
    );
  };
